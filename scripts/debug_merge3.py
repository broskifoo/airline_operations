#!/usr/bin/env python3
"""Debug the T-100 data after filtering."""
import pandas as pd
import zipfile
...
if 'service_class' in t100.columns:
    ...
    print("Columns:", list(t100.columns))
    
    # Check for origin/dest
    print("Has origin_airport:", 'origin_airport' in t100.columns)
    print("Has dest_airport:", 'dest_airport' in t100.columns)
    
    # Create route
    if 'origin_airport' in t100.columns and 'dest_airport' in t100.columns:
        t100['route'] = t100['origin_airport'] + '-' + t100['dest_airport']
        print("Created route column")
        print("Sample routes:", t100['route'].unique()[:10])
    else:
        print("Missing origin_airport or dest_airport")